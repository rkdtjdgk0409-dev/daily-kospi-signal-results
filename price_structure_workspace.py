...
.workspace{{display:grid;grid-template-columns:minmax(0,1fr) 390px;gap:10px;padding:10px;max-width:1920px;width:100%;margin:0 auto;flex:1;min-height:0}}.chart-panel,.rail{{border:1px solid var(--line);background:rgba(15,20,27,.96);border-radius:12px;overflow:hidden}}.chart-panel{{display:flex;flex-direction:column;min-width:0}}.chart-head{{display:flex;align-items:center;justify-content:space-between;gap:10px;padding:8px 10px;border-bottom:1px solid var(--line);background:#0e141c}}.chart-title{{min-width:0}}.chart-title strong{{font-size:13px}}.chart-title span{{display:block;color:var(--muted);font-size:9px;margin-top:3px}}.range,.chart-tools{{display:flex;gap:5px;align-items:center;flex-wrap:wrap}}.range-btn,.toggle-btn,.chart-tool{{border:1px solid var(--line);background:#111821;color:#c8d2dc;border-radius:6px;padding:6px 8px;font-size:10px;cursor:pointer;line-height:1}}.range-btn:hover,.toggle-btn:hover,.chart-tool:hover{{border-color:#536274;background:#17212b}}.toggle-btn.active,.chart-tool.active{{border-color:rgba(41,184,176,.62);background:rgba(41,184,176,.13);color:#e9fffc}}.ohlcbar{{min-height:39px;display:flex;align-items:center;gap:11px;overflow:auto;white-space:nowrap;padding:6px 10px;border-bottom:1px solid var(--line);background:#0a0f15;font-variant-numeric:tabular-nums;scrollbar-width:none}}.ohlcbar::-webkit-scrollbar{{display:none}}.ohlc-symbol{{font-size:11px;font-weight:900;color:#eef3f8}}.ohlc-date{{font-size:10px;color:var(--muted)}}.ohlc-item{{font-size:10px;color:#b7c2cd}}.ohlc-item b{{color:#edf2f7;font-weight:800}}.ohlc-change{{font-size:10px;font-weight:850}}.layerbar{{display:flex;align-items:center;justify-content:space-between;gap:8px;padding:6px 9px;border-bottom:1px solid var(--line);background:#0b1016}}.layers{{display:flex;align-items:center;gap:5px;overflow:auto;scrollbar-width:none}}.layers::-webkit-scrollbar{{display:none}}.layer-label{{font-size:10px;color:var(--muted);margin-right:2px}}.chart-help{{font-size:9px;color:#728090;white-space:nowrap}}#chart{{height:calc(100vh - 183px);min-height:570px;width:100%;touch-action:none;position:relative}}
...
@media(max-width:850px){{.topbar{{height:auto;min-height:56px;padding:8px 10px;align-items:flex-start}}.nav{{overflow:auto;max-width:58vw}}.workspace{{display:block;padding:8px}}.chart-panel{{margin-bottom:8px}}#chart{{height:64vh;min-height:440px}}.rail{{max-height:none}}.company{{max-width:140px}}.chart-help{{display:none}}}}
@media(max-width:520px){{.topbar{{position:relative}}.nav a:nth-child(2),.nav a:nth-child(3){{display:none}}.workspace{{padding:6px}}.chart-head{{display:block}}.range{{margin-top:7px;overflow:auto;flex-wrap:nowrap;scrollbar-width:none}}.range::-webkit-scrollbar{{display:none}}.ohlcbar{{gap:9px;padding:6px 8px}}.layerbar{{overflow:hidden}}.layers{{flex:1}}.layer-label{{display:none}}#chart{{height:58vh;min-height:390px}}.rail-inner{{padding:10px}}.chart-tool,.range-btn,.toggle-btn{{padding:7px 9px;font-size:10px}}}}
</style>
</head>
<body><div class="app">
<header class="topbar">
...
<main class="workspace">
  <section class="chart-panel">
    <div class="chart-head"><div class="chart-title"><strong>차트 구조</strong><span>M-able형 캔들 조작 · 드래그 이동 · 휠/핀치 확대 · 십자선 OHLC 확인</span></div><div class="range"><button class="range-btn" onclick="setRangeBars(44)">2M</button><button class="range-btn" onclick="setRangeBars(66)">3M</button><button class="range-btn" onclick="setRangeBars(132)">6M</button><button class="range-btn" onclick="setRangeBars(252)">1Y</button><button class="range-btn" onclick="setRangeBars(CHART_N)">전체</button></div></div>
    <div class="ohlcbar" id="ohlcbar"><span class="ohlc-symbol" id="ohlcSymbol">-</span><span class="ohlc-date" id="ohlcDate">-</span><span class="ohlc-item">시 <b id="ohlcOpen">-</b></span><span class="ohlc-item">고 <b id="ohlcHigh">-</b></span><span class="ohlc-item">저 <b id="ohlcLow">-</b></span><span class="ohlc-item">종 <b id="ohlcClose">-</b></span><span class="ohlc-change" id="ohlcChange">-</span></div>
    <div class="layerbar"><div class="layers"><span class="layer-label">표시</span><button class="toggle-btn active" data-layer="zones">매물대</button><button class="toggle-btn active" data-layer="wave">파동</button><button class="toggle-btn active" data-layer="channel">채널</button><button class="toggle-btn active" data-layer="execution">매매포인트</button><button class="toggle-btn active" data-layer="forecast">예상경로</button><button class="toggle-btn active" data-layer="fib">Fib</button></div><div class="chart-tools"><button class="chart-tool active" id="panBtn" onclick="setDragMode('pan')">이동</button><button class="chart-tool" id="zoomBtn" onclick="setDragMode('zoom')">영역확대</button><button class="chart-tool" onclick="zoomChart(.72)" title="캔들 확대">＋</button><button class="chart-tool" onclick="zoomChart(1.38)" title="캔들 축소">－</button><button class="chart-tool" onclick="fitVisiblePrice()">가격맞춤</button><button class="chart-tool" onclick="resetChart()">초기화</button><button class="chart-tool" onclick="toggleFullscreen()">⛶</button></div><span class="chart-help">십자선에 마우스를 올리면 한 봉 가격 표시</span></div>
    <div id="chart"></div>
  </section>
...
let CURRENT=null,CHART_N=0,CHART_FUTURE=0,CHART_BARS=[],CHART_CATS=[],VIEW_RANGE=null,INTERNAL_RELAYOUT=false,DRAG_MODE='pan',WHEEL_BOUND=false;
...
function setTitle(m){{topSymbol.textContent=m.ticker||'-';topCompany.textContent=m.name||'-';ohlcSymbol.textContent=m.ticker||m.name||'-';document.title=`${{m.name||m.ticker}} · Chart Structure`}}
function candleChange(bar){{if(!bar)return null;const o=Number(bar.open),c=Number(bar.close);return o?((c/o)-1)*100:null}}
function updateOhlc(bar){{if(!bar)return;ohlcDate.textContent=bar.date||'-';ohlcOpen.textContent=money(bar.open);ohlcHigh.textContent=money(bar.high);ohlcLow.textContent=money(bar.low);ohlcClose.textContent=money(bar.close);const ch=candleChange(bar);ohlcChange.textContent=ch==null?'-':`${{ch>=0?'+':''}}${{ch.toFixed(2)}}%`;ohlcChange.style.color=ch>0?'#ff5966':ch<0?'#4a8dff':'#9aa5b0'}}
function currentRange(){{if(VIEW_RANGE&&VIEW_RANGE.length===2)return VIEW_RANGE.slice();const c=CURRENT?.analysis?.chart;if(!c?.bars?.length)return null;return initialRange(c,c.bars,CHART_FUTURE)}}
function clampRange(r){{if(!r||!CHART_N)return r;let a=Number(r[0]),b=Number(r[1]);if(!Number.isFinite(a)||!Number.isFinite(b))return currentRange();const min=-.5,max=CHART_N+CHART_FUTURE-.5,width=Math.max(8,b-a);if(a<min){{b+=min-a;a=min}}if(b>max){{a-=b-max;b=max}}a=Math.max(min,a);b=Math.min(max,b);if(b-a<8){{const mid=(a+b)/2;a=mid-4;b=mid+4}}return [a,b]}}
function visibleBarSlice(r=currentRange()){{if(!CHART_BARS.length)return [];r=clampRange(r)||[-.5,CHART_BARS.length-.5];const lo=Math.max(0,Math.floor(r[0]+.5)),hi=Math.min(CHART_BARS.length-1,Math.ceil(r[1]-.5));return CHART_BARS.slice(lo,hi+1)}}
function visiblePriceRange(r=currentRange()){{const v=visibleBarSlice(r);if(!v.length)return null;let lo=Math.min(...v.map(x=>Number(x.low)).filter(Number.isFinite)),hi=Math.max(...v.map(x=>Number(x.high)).filter(Number.isFinite));if(!Number.isFinite(lo)||!Number.isFinite(hi))return null;let pad=Math.max((hi-lo)*.08,Math.abs(hi)*.008,1);return [lo-pad,hi+pad]}}
async function applyViewRange(r,fitY=true){{r=clampRange(r);if(!r)return;VIEW_RANGE=r;const upd={{'xaxis.range':r}};if(fitY){{const yr=visiblePriceRange(r);if(yr)upd['yaxis.range']=yr}}INTERNAL_RELAYOUT=true;try{{await Plotly.relayout('chart',upd)}}finally{{INTERNAL_RELAYOUT=false}}}}
function setDragMode(mode){{DRAG_MODE=mode==='zoom'?'zoom':'pan';panBtn.classList.toggle('active',DRAG_MODE==='pan');zoomBtn.classList.toggle('active',DRAG_MODE==='zoom');Plotly.relayout('chart',{{dragmode:DRAG_MODE}}).catch(()=>{{}})}}
function zoomChart(factor,center=null){{const r=currentRange();if(!r)return;const width=Math.max(8,r[1]-r[0]);const nw=Math.min(CHART_N+CHART_FUTURE,Math.max(12,width*factor));const mid=center==null?(r[0]+r[1])/2:Number(center);applyViewRange([mid-nw/2,mid+nw/2],true)}}
function panChart(frac){{const r=currentRange();if(!r)return;const d=(r[1]-r[0])*frac;applyViewRange([r[0]+d,r[1]+d],true)}}
function fitVisiblePrice(){{const yr=visiblePriceRange();if(!yr)return;INTERNAL_RELAYOUT=true;Plotly.relayout('chart',{{'yaxis.range':yr}}).finally(()=>INTERNAL_RELAYOUT=false)}}
function resetChart(){{VIEW_RANGE=null;const c=CURRENT?.analysis?.chart;if(!c)return;const r=initialRange(c,CHART_BARS,CHART_FUTURE);applyViewRange(r,true);setDragMode('pan')}}
function toggleFullscreen(){{const el=document.querySelector('.chart-panel');if(!document.fullscreenElement)el.requestFullscreen?.();else document.exitFullscreen?.()}}
function bindChartInteractions(){{const gd=document.getElementById('chart');if(!gd)return;if(!WHEEL_BOUND){{WHEEL_BOUND=true;gd.addEventListener('wheel',e=>{{if(!CHART_N)return;e.preventDefault();const r=currentRange();if(!r)return;const rect=gd.getBoundingClientRect();const x=Math.min(1,Math.max(0,(e.clientX-rect.left)/Math.max(1,rect.width)));const center=r[0]+(r[1]-r[0])*x;zoomChart(e.deltaY>0?1.16:.86,center)}},{{passive:false,capture:true}})}}gd.removeAllListeners?.('plotly_hover');gd.removeAllListeners?.('plotly_unhover');gd.removeAllListeners?.('plotly_relayout');gd.on('plotly_hover',ev=>{{const p=ev?.points?.find(x=>x.data?.type==='candlestick')||ev?.points?.[0];if(p&&Number.isFinite(p.pointNumber))updateOhlc(CHART_BARS[p.pointNumber])}});gd.on('plotly_unhover',()=>{{if(CHART_BARS.length)updateOhlc(CHART_BARS[CHART_BARS.length-1])}});gd.on('plotly_relayout',ev=>{{if(INTERNAL_RELAYOUT)return;let r=null;if(Array.isArray(ev['xaxis.range']))r=ev['xaxis.range'];else if(ev['xaxis.range[0]']!=null&&ev['xaxis.range[1]']!=null)r=[ev['xaxis.range[0]'],ev['xaxis.range[1]']];if(r){{VIEW_RANGE=clampRange(r);fitVisiblePrice()}}else if(ev['xaxis.autorange']){{VIEW_RANGE=null}}}})}}
...
function setRangeBars(count){{if(!CHART_N)return;const c=Math.max(12,Math.min(Number(count)||CHART_N,CHART_N));applyViewRange([Math.max(-.5,CHART_N-c-.5),CHART_N+CHART_FUTURE-.5],true)}}
function drawChart(c,name){{if(!c?.bars?.length){{chart.innerHTML='<div class="loading">차트 데이터가 없습니다.</div>';return}}const bars=c.bars,dates=bars.map(x=>x.date),w=c.wave||{{}};CHART_BARS=bars;CHART_N=bars.length;let future=[];if(SHOW.forecast&&w.forecast?.length){{const last=bars[bars.length-1];future=w.forecast.map(p=>{{const delta=Math.max(0,Math.round(Number(p.x)-Number(last.x)));return delta===0?last.date:futureBusinessDate(last.date,delta)}}).filter(x=>!dates.includes(x));future=[...new Set(future)]}}CHART_FUTURE=future.length;const cats=[...dates,...future];CHART_CATS=cats;const traces=[],shapes=[],ann=[],x0=dates[0],x1=dates[dates.length-1],sm=mobileMode();addParallelChannel(traces,ann,bars,c.parallel_channel);traces.push({{type:'candlestick',x:dates,open:bars.map(x=>x.open),high:bars.map(x=>x.high),low:bars.map(x=>x.low),close:bars.map(x=>x.close),name,increasing:{{line:{{color:'#ff5966',width:1.35}},fillcolor:'#ff5966'}},decreasing:{{line:{{color:'#4a8dff',width:1.35}},fillcolor:'#4a8dff'}},whiskerwidth:.55,hoverinfo:'x+name',showlegend:false}});function zone(z,color,label){{if(!SHOW.zones||z?.low==null||z?.high==null)return;shapes.push({{type:'rect',xref:'x',yref:'y',x0,x1,y0:z.low,y1:z.high,fillcolor:color,opacity:.085,line:{{width:1,color}}}});ann.push({{x:x1,y:z.center??((Number(z.low)+Number(z.high))/2),text:label,showarrow:false,xanchor:'right',font:{{size:sm?8:10,color}},bgcolor:'rgba(8,11,16,.66)'}})}}zone(c.support,'#27d79a','SUPPORT');zone(c.resistance,'#ff6474','RESIST');if(SHOW.zones&&c.poc!=null)shapes.push({{type:'line',xref:'x',yref:'y',x0,x1,y0:c.poc,y1:c.poc,line:{{color:'#a78bfa',width:1,dash:'dot'}}}});addFib(shapes,ann,bars,w);addExecution(shapes,ann,bars,c.execution_plan||{{}});addWave(traces,ann,w);addForecast(traces,ann,bars,w);const last=bars[bars.length-1];if(last?.close!=null){{const up=Number(last.close)>=Number(last.open),pc=up?'#ff5966':'#4a8dff';shapes.push({{type:'line',xref:'paper',yref:'y',x0:0,x1:1,y0:last.close,y1:last.close,line:{{color:pc,width:1,dash:'dot'}}}});ann.push({{xref:'paper',x:1,y:last.close,text:money(last.close),showarrow:false,xanchor:'left',xshift:4,font:{{size:10,color:'#fff'}},bgcolor:pc,borderpad:3}})}}const firstRange=clampRange(VIEW_RANGE)||initialRange(c,bars,CHART_FUTURE),yr=visiblePriceRange(firstRange);Plotly.newPlot('chart',traces,{{paper_bgcolor:'transparent',plot_bgcolor:'#0b1016',font:{{color:'#8b98a7'}},margin:{{l:4,r:sm?78:92,t:7,b:sm?28:34}},xaxis:{{type:'category',categoryorder:'array',categoryarray:cats,range:firstRange,rangeslider:{{visible:false}},showgrid:true,gridcolor:'#1b242d',gridwidth:1,zeroline:false,automargin:true,tickfont:{{size:sm?9:10}},ticks:'outside',ticklen:4,fixedrange:false,showspikes:true,spikemode:'across',spikesnap:'cursor',spikecolor:'#697684',spikethickness:1,spikedash:'dot'}},yaxis:{{gridcolor:'#1b242d',gridwidth:1,side:'right',tickformat:CURRENCY.decimals?',.2f':',.0f',tickprefix:CURRENCY.symbol,separatethousands:true,ticks:'outside',ticklen:4,tickfont:{{size:sm?9:10}},automargin:true,zeroline:false,fixedrange:false,range:yr||undefined,showspikes:true,spikemode:'across',spikesnap:'cursor',spikecolor:'#697684',spikethickness:1,spikedash:'dot'}},shapes,annotations:ann,showlegend:false,hovermode:'x',hoverdistance:-1,spikedistance:-1,dragmode:DRAG_MODE,hoverlabel:{{bgcolor:'#111821',bordercolor:'#3a4654',font:{{color:'#eef3f8',size:11}},namelength:0}},uirevision:'mable-chart'}},{{displayModeBar:false,responsive:true,scrollZoom:false,doubleClick:false}}).then(()=>{{VIEW_RANGE=firstRange;updateOhlc(last);bindChartInteractions();setDragMode(DRAG_MODE)}})}}
