...
def load_state(path: Path) -> dict:
    if not path.exists():
        return {"version": "US-1.0", "positions": {}, "closed": []}
    try:
        x = json.loads(path.read_text(encoding="utf-8"))
        x.setdefault("version", "US-1.0")
...
def classify(r, pnl, risk, trail):
    ...
    if close <= trail:
        return "STOP", "Close fell below the volatility-adjusted trailing stop."
    if risk >= 75 or ((not st_good) and mdi > pdi and alpha < 55):
        return "EXIT", "Trend reversal + DMI/Alpha deterioration overlap."
    if pnl >= 6 and (risk >= 55 or (cci >= 140 and cci_d < 0) or not st_good):
        return "TAKE PROFIT", "Profitable position with slowing trend; partial profit candidate."
    if risk >= 45:
        return "WATCH", "Trend deterioration increased; defense before adding."
    if risk < 20 and st_good and pdi > mdi and alpha >= 70:
        return "STRONG HOLD", "Trend, DMI and Alpha remain aligned."
    return "HOLD", "Core trend remains intact; manage with trailing stop."
...
def main():
    ...
    ap.add_argument("--input", default="us_results/latest_all_scored.csv")
    ap.add_argument("--state", default="state/us_position_state.json")
    ap.add_argument("--output", default="us_results/latest_position_management.csv")
    a = ap.parse_args()

    df = pd.read_csv(Path(a.input), dtype={"ticker": str})
    ...
    closed = state["closed"]
    rows = {str(r["ticker"]).upper(): r for _, r in df.iterrows()}
    ...
    state["updated_at"] = datetime.now(NY).isoformat(timespec="seconds")
    state_path.parent.mkdir(parents=True, exist_ok=True)
    state_path.write_text(json.dumps(state, ensure_ascii=False, indent=2), encoding="utf-8")

    print("[us-position] rows:", len(odf))
