...
<title>Korea Wave Structure Scanner</title>
...
<div class="top">
  ...
  <div class="actions"><a class="market-link" href="../">한국 시장</a><a class="market-link" href="../us/">미국 시장</a><a class="market-link" href="../position/">포지션 관리</a><button class="btn" onclick="location.reload()">새로고침</button></div>
</div>
<div class="cards" id="cards"></div>
<div class="toolbar">
  <input id="search" class="search" placeholder="종목명 또는 코드 검색"/>
  <select id="market" class="select"><option value="ALL">전체 시장</option><option>KOSPI</option><option>KOSDAQ</option></select>
...
function krw(v,d=0){return v==null||Number.isNaN(+v)?'-':'₩'+Number(v).toLocaleString('ko-KR',{maximumFractionDigits:d})}
...
function filtered(){const q=document.getElementById('search').value.trim().toLowerCase(),m=document.getElementById('market').value,g=document.getElementById('grade').value,s=document.getElementById('sort').value;let a=DATA.filter(r=>(!q||String(r.name).toLowerCase().includes(q)||String(r.ticker).toLowerCase().includes(q))&&(m==='ALL'||r.market===m)&&(g==='ALL'||r.grade===g)&&(FILTER==='ALL'||r.setup===FILTER));a.sort((x,y)=>(Number(y[s])||-999)-(Number(x[s])||-999));return a;}
...
<div class="metrics">${metric('현재가',krw(a.price))}${metric('파동 상태',w.label||'-',setupCls)}${metric('파동 신뢰도',`${n(w.confidence,0)}%`,w.confidence>=72?'bull':'')}${metric('지지 구간',sup.low!=null?`${krw(sup.low)} ~ ${krw(sup.high)}`:'-')}${metric('저항 구간',res.low!=null?`${krw(res.low)} ~ ${krw(res.high)}`:'-')}${metric('평행채널',channelMetric,ch.direction==='ASCENDING'?'bull':ch.direction==='DESCENDING'?'watch':'')}${metric('확인 가격',krw(sc.confirm_price))}${metric('무효화',krw(sc.invalidation_price),'bear')}${metric('목표 1 / 2',`${krw(sc.target1)} / ${krw(sc.target2)}`,'bull')}</div>
<div class="scenario"><div class="scenario-title">${w.label||'구조 시나리오'}</div><div class="scenario-line"><div class="scenario-num">01</div><div>${sc.thesis||'구조 확인 중'}</div></div><div class="scenario-line"><div class="scenario-num">02</div><div><span class="scenario-key">대응:</span> ${sc.action||'-'}</div></div><div class="scenario-levels"><span class="level-pill">Entry zone ${ez.low!=null?krw(ez.low)+' ~ '+krw(ez.high):'-'}</span><span class="level-pill">Confirm ${krw(sc.confirm_price)}</span><span class="level-pill">Invalid ${krw(sc.invalidation_price)}</span><span class="level-pill">RVOL ${n(b.rvol,2)}</span><span class="level-pill">RS ${n(rs.score)}</span><span class="level-pill">R/R ${n(tr.rr1,2)}</span></div></div>
...
def main() -> None:
    ...
    if not (src / "summary.json").exists():
        raise SystemExit("price_structure_results/summary.json not found. Run price_structure_scanner.py first.")
