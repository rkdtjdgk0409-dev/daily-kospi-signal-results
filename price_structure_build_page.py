...
<div class="top">
  ...
  <div class="actions"><a class="market-link" href="../">한국 지표</a><a class="market-link" href="../us/">미국 시장</a><a class="market-link" href="../position/">포지션 관리</a><button class="btn" onclick="location.reload()">새로고침</button></div>
</div>
...
function filtered(){
  ...
  let a=DATA.filter(r=>(!q||r.name.toLowerCase().includes(q)||r.ticker.includes(q))&&(m==='ALL'||r.market===m)&&(g==='ALL'||r.grade===g)&&(FILTER==='ALL'||r.setup===FILTER));
...
function showDetail(d){
  ...
  document.getElementById('detailPanel').innerHTML=`<div class="detail">
    ...
    <div class="metrics">
      ${metric('현재가',krw(a.price,0))}${metric('시장 구조',a.structure?.code||'-')}${metric('Breakout Quality',n(b.score))}
    ...
    <div class="sectiontitle">Trade Structure</div><div class="metrics">${metric('Entry',krw(tr.entry,0))}${metric('Stop / Invalidation',krw(tr.stop,0))}${metric('Target 1',krw(tr.target1,0))}${metric('Target 2',krw(tr.target2,0))}${metric('Risk (ATR)',n(tr.risk_atr,2))}${metric('Hard Filter',a.hard_filter_pass?'PASS':'WATCH',a.hard_filter_pass?'bull':'watch')}</div>
  </div>`;
...
function drawChart(c,name){
  ...
  Plotly.newPlot('chart',traces,{
    ...
    yaxis:{
      gridcolor:'#18233a',side:'right',tickformat:',.0f',ticksuffix:'원',
